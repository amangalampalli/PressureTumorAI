...
def drawPaperTemplate():
    ...
    def drawMarkerAt(id, markerCenter):
        ...
        paddedMarker[
            padding : (facePx * 2) - padding, padding : (facePx * 2) - padding
        ] = marker
        cv2.rectangle(
            paddedMarker,
            (0, 0),
            (paddedMarker.shape[0] - 1, paddedMarker.shape[1] - 1),
            0,
            1,
        )
        cubeTemplate[
            markerCenter[1] - facePx : markerCenter[1] + facePx,
            markerCenter[0] - facePx : markerCenter[0] + facePx,
        ] = paddedMarker

    drawMarkerAt(board_ids[0], (int(cubeTemplate.shape[1] / 2), facePx))
    drawMarkerAt(
        board_ids[1], (int(cubeTemplate.shape[1] / 2), facePx + 1 * facePx * 2)
    )
    drawMarkerAt(
        board_ids[2],
        (int(cubeTemplate.shape[1] / 2) - facePx * 2, facePx + 2 * facePx * 2),
    )
    drawMarkerAt(
        board_ids[3], (int(cubeTemplate.shape[1] / 2), facePx + 2 * facePx * 2)
    )
    drawMarkerAt(
        board_ids[4],
        (int(cubeTemplate.shape[1] / 2) + facePx * 2, facePx + 2 * facePx * 2),
    ...
    drawMarkerAt(
        board_ids[5], (int(cubeTemplate.shape[1] / 2), facePx + 3 * facePx * 2)
    )
    return cubeTemplate
...
cv2.imwrite(
    "/Users/aditya/Programming/PressureTumorAI/pressuretumorai/aruco_markers/markers/cubeNet.png",
    cv2.resize(drawPaperTemplate(), None, fx=2, fy=2),
)
