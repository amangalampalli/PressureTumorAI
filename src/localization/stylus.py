...
def processFrame(findPosition=False):
    global rvec, tvec, board, tipMatrix
    # ret, frame = cap.read()
    ...
    if retval > 0:
        if findPosition == False:
            ...
            return frame
        elif findPosition == True:
            rMatrix = cv2.Rodrigues(rvec)[0]
            rotationTipMatrix = np.matmul(rMatrix, tipMatrix)
            translationTipMatrix = np.add(rotationTipMatrix, np.transpose(tvec)[0])
            return translationTipMatrix
    else:
        if findPosition == False:
            return frame
        elif findPosition == True:
...
if __name__ == "__main__":
    print(processFrame(findPosition=True))
